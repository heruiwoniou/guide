...
function When2do(condition) {
  var option = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : { times: 10 };
...
var when = function when(condition, option) {
  return new When2do(condition, option);
};

var location = function location(el) {
  var result = {
    x: el.offsetLeft,
    y: el.offsetTop
  };
...
Guide.prototype = {
  ...
  init: function init() {
    ...
    this.queue.forEach(function (o) {
      requires.push(new Promise(function (resolve, reject) {
        var img = new Image();
        img.onload = function () {
          o.img = img;
          resolve();
  ...
  drawItem: function drawItem(isresize) {
    var _this4 = this;

    if (this.doing || !this.option) return;
    this.doing = true;
    var option = this.option;
    var handler = option.handler || function () {
      return { x: 0, y: 0 };
    };
    var src = option.src;
    ...
    var before = option.before || function (done) {
      done();
    };

    when(function () {
      return document.querySelector(selector);
    }, { times: repeat }).then(function () {
      setTimeout(function () {
        var todo = function todo() {
          ...
          img.addEventListener('load', function () {
            var offset = handler(el, img, option),
                loc = location(el),
                x = loc.x + offset.x,
                y = loc.y + offset.y;
  ...
  next: function next() {
    ...
    if (this.queue.length === 0) this.dispose();else {
